...
function CharacterDropDown() {
  ...
  useEffect(() => {
    ...
    async function getCharacters() {
      const response = await fetch("/api/audit/menulist");
      const body = await response.json();
      if (!unmounted) {
        setItems(
          body.data.map(({ course, instructor, classtime, cohort,  ...item }) => ({
            label: `${item._id.course} | ${item._id.instructor} | ${item._id.classtime} | ${item._id.cohort}`,
            value: `${item._id.course} | ${item._id.instructor} | ${item._id.classtime} | ${item._id.cohort}`,
