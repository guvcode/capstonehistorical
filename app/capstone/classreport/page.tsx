...
import { FaCheck, FaTimes, FaCheckDouble } from "react-icons/fa";
import Image from "next/image";

const presentList = ["PC", "P", "CP", "CCCP", "PCC", "PA", "AP", "PP", "PRESENT"];
...
const transpose = (matrix: studentRecord[]) => {
  ...
  for (const item of matrix) {
    ...
    let absentFromClass = 0;
    let attendanceList = "";
    if (presentList.includes(item.attendanceStatus.trim().toUpperCase())) {
      presentInclass++;
    } else {
      absentFromClass++;
    }
    ...
    if (aRow) {
      aRow[item.classDate.slice(0, 10)] = item.attendanceStatus;
      attendanceList = attendanceList.concat(item.attendanceStatus);

      aRow.present = aRow.present + attendanceList.replace(/[^p]/gi, "").length; // Updated this line to use "+=" instead of "="
      aRow.absent = aRow.absent + attendanceList.replace(/[^a]/gi, "").length; // Updated this line to use "+=" instead of "="
      attendanceList += item.attendanceStatus;
      updateGridHeader(item.classDate.slice(0, 10));
    } else {
      attendanceList = item.attendanceStatus;
      result.push({
        name: titleCase(item.name),
        email: item.email,
        present: attendanceList.replace(/[^p]/gi, "").length,
        absent: attendanceList.replace(/[^a]/gi, "").length,
        [item.classDate.slice(0, 10)]: item.attendanceStatus,
      });
      updateGridHeader(item.classDate.slice(0, 10));
    }
  }
...
const updateGridHeader = (classDate) => {
  const newHeader = {
    key: classDate,
    title: classDate,
    width: 60,
    dataType: DataType.String,
    sortDirection: DataType.String,
    isSortable: false,
  };
  let gridColumn = gridColumns.find((header) => header.key === classDate);
...
const ClassAttendance = () => {
  ...
  useEffect(() => {
    ...
    fetch(`/api/class/${course}/${time}/${tutor}/attendance`)
      .then((res) => res.json())
      .then((data) => {
        setData(data);
        setgridDate(transpose(data.data.doc));
        setLoading(false);
      });
  }, []);
            ...
            <div className="w-full px-4">
              <div className="mx-auto max-w-[1200px] text-center">
                ...
                <div className="w-full table-auto overflow-x-scroll">
                  {studentlistHeadertable(data)}

                  {studentListTable(gridData)}
                </div>
              </div>
            </div>
          </div>
...
function studentListTable(gridData: any) {
  const numberofbreaks = Math.floor((gridColumns.length + 5) / 6);
  const newGridColumns = chunkIntoN(gridColumns, numberofbreaks);
...
function studentlistHeadertable(data: any) {
