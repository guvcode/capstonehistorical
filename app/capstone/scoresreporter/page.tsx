...
function CharacterDropDown() {
  ...
  const handleClick = () => {
    ...
    // redirect(`/classreport?course=${valueArray[0]}&tutor=${valueArray[1]}/&time=${valueArray[2]}`);
    // if (!(typeof window === undefined)) {
    router.push(
      `/capstone/classreport/scores?course=${valueArray[0].trim()}&tutor=${valueArray[1].trim()}&time=${valueArray[2].trim()}`
    );
    //}
  };
  ...
  useEffect(() => {
    ...
    async function getCharacters() {
      ...
      if (!unmounted) {
        setItems(
          body.data.map(({ course, instructor, classtime, ...item }) => ({
            label: `${item._id.course} | ${item._id.instructor} | ${item._id.classtime}`,
            value: `${item._id.course} | ${item._id.instructor} | ${item._id.classtime}`,
            course: item._id.course,
            instructor: item._id.instructor,
            classtime: item._id.classtime,
          }))
        );
        setLoading(false);
      }
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 gap-x-8 gap-y-10 md:grid-cols-2 lg:grid-cols-3">
        {items.map((item) => (
          <SingleTestimonial key={item.id} item={item} />
        ))}
      </div>
    </>
  );
